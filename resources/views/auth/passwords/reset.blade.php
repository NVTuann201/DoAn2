      ...
      <div class="white-box">
        ...
        <form class="form-horizontal" method="POST" action="{{ route('password.reset') }}">
          ...
          <div class="form-group ">
            <div class="col-xs-12">
              <h3>{{__('page.login.setup_password')}}</h3>
            </div>
          </div>
          <div class="form-group ">
            <div class="col-xs-12">
              <input class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" type="email" placeholder="Email" name="email" value="{{ $email ?? old('email') }}" required readonly>
              @if ($errors->has('email'))
          ...
          <div class="form-group">
            <div class="col-xs-12">
                <input class="form-control {{ $errors->has('password') ? ' is-invalid' : '' }}" type="password" placeholder="Mật khẩu" name="password" value="{{ $password ?? old('password') }}" required>
          ...
          <div class="form-group">
            <div class="col-xs-12">
                <input class="form-control {{ $errors->has('password_confirmation') ? ' is-invalid' : '' }}" type="password" placeholder="Nhập lại mật khẩu" name="password_confirmation" value="{{ $password_confirmation ?? old('password_confirmation') }}" required>
            ...
            <div class="col-xs-12">
              <button class="btn btn-primary btn-lg btn-block text-uppercase waves-effect waves-light" style="background-color: #016299" type="submit">{{__('page.login.confirm')}}</button>
            </div>
          </div>
        </form>
      </div>
