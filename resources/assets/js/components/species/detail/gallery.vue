  ...
  <div>
    <div id="mygallery" class="imageGallery">
      <template v-for="item in images">
        ...
        >
          <img
            class="imgGallery"
            :alt="item.title || item.darwin_core_taxon.scientific_name"
            :src="`/storage/resources/images/${item.identifier}`"
            v-show="false"
          />
          <div>{{ item.title || item.darwin_core_taxon.scientific_name }}</div>
        </a>
        <a
          v-else
          :key="item.id"
          :style="`background-image: url('${item.media[0].thumbnail}')`"
        >
          <img
            class="imgGallery"
            :alt="item.title || item.darwin_core_taxon.scientific_name"
    ...
    <section
      ...
      style="
        ...
        height: calc(100vh - 40.117142857129285714285714285714rem);
      "
    >
...
export default {
  ...
  methods: {
    getData() {
      ...
      axios
        .get(`${env.endpointhttp}detail/indexdataset/images/${this.id}`, {
          params: {
