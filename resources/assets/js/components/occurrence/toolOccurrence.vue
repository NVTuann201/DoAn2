                                    ...
                                    <div class="container--normal" style="max-width: 1155px">
                                        <div class="scrollable">
                                            <div class="table-responsive">
                                                <h3>{{$t("tool.occurrence_data")}}</h3>
                                                <p>{{$t("tool.data_take")}} <a href="https://www.gbif.org/developer/occurrence" target="_blank">https://www.gbif.org/developer/occurrence</a></p>
                                                <table class="table table-bordered table-striped m-t-30 table-hover contact-list footable footable-1 footable-paging footable-paging-center breakpoint-lg">
                                                    <thead>
                                                        <tr class="footable-header">
                                                            ...
                                                            <th>{{$t("tool.dataset_name")}}</th>
                                                            <th style="vertical-align: middle">{{$t("tool.coordinates")}}</th>
                                                            <th style="vertical-align: middle">{{$t("tool.date")}}</th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr v-if="occurrences && occurrences.length == 0">
                                                            <td colspan="11" class="emptyInfomation">
                                                                <h5>{{ $t("admin.error.no_data") }}</h5>
                                                            </td>
                                                        </tr>
                                                        <tr v-for="(item, index) in occurrences">
                                                            <td>
                                                            ...
                                                            <td class="position-relative">
                                                                <span class="position-absolute center-vertical">
                                                                    ...
                                                                    <br>
                                                                    <span class="text-muted">{{item.decimalLongitude}}</span>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <span v-if="item.day && item.month && item.year">
                                        ...
                                        <tr class="footable-paging" v-if="occurrences && occurrences.length != 0">
                                            <td colspan="6">
                                                <paginate v-if="count>1" v-model="page" :page-count="this.count" :page-range="3"
        ...
        methods: {
            ...
            searchAll() {
                axios
                    ...
                    .then(response => {
                        this.occurrences = response.data.results
                        this.count = response.data.count <= 199980 ? Math.ceil(response.data.count/20):9999
                        this.page = 1
                    })
            }
        },
