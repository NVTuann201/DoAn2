      ...
      <div class="right-aside">
        <div class="d-flex">
          <div style="flex: 1; font-size: 18px; font-weight: bold">
            Danh sách hành lang đa dạng sinh học
          </div>
          <div class="d-flex">
            <input
              class="form-control"
            ...
            >
              <thead>
                <tr class="footable-header">
                  ...
                  <th style="display: table-cell">Mục đích</th>
                  <th style="display: table-cell; min-width: 70px">Mô tả</th>
                  <th style="display: table-cell; min-width: 70px">
                    Tình trạng
                  </th>
                  <th style="display: table-cell">Cấp quản lý</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td>
                    <div>
                      {{ item.tinh_trang ? item.tinh_trang.name : "" }}
                    </div>
                  </td>
                  <td>
                    <div>
                      {{ item.quan_ly ? item.quan_ly.name : "" }}
                    </div>
                  </td>
                  <td>
                    <button
...
export default {
  ...
  methods: {
    showXoa(data) {
      this.contentDelete =
        "Bạn có chắc chắn muốn xóa hành lang đa dạng sinh học " +
        data.ten +
        " không?";
      this.idDelete = data.id;
      this.confirmDelete = true;
      this.delLink = "admin/hanhlangdadangsinhhoc/delete/";
    },
    xoaDuLieu() {
      axios
    ...
    showFormAdd() {
      window.location.href = "/admin/hanhlangdadangsinhhoc/add";
    },
    showFormEdit(data) {
      window.location.href = "/admin/hanhlangdadangsinhhoc/edit/" + data.id;
    },
    getData() {
      axios
        .get(env.endpointhttp + "admin/hanhlangdadangsinhhoc/get", {
          params: {
            page: this.page,
            search: this.search,
    ...
    showXoaBoLoc() {
      if (this.filter.tinh_trang_id == null && this.filter.quan_ly_id == null) {
        return false;
      } else return true;
    },
    deleteSearchFilter() {
    ...
    getChiSo() {
      axios
        .get(env.endpointhttp + "admin/hanhlangdadangsinhhoc/chiso")
        .then((res) => {
