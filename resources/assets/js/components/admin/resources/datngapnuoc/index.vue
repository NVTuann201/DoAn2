        ...
        <div class="scrollable">
          ...
          <ul class="list-style-none" style="background: white">
            ...
            <li
              ...
              v-for="item in dataFilter.tinh_trang"
              :key="item.id"
            >
              <a
                @click="searchType(item.id, 'tinh_trang_id')"
                style="cursor: pointer"
                :style="
                  filter.tinh_trang_id == item.id
            ...
            <li
              ...
              style="margin-left: 15px"
              v-for="item in dataFilter.do_quan_trong"
              :key="item.id"
            >
              <a
                @click="searchType(item.id, 'do_quan_trong_id')"
                style="cursor: pointer"
                :style="
                  filter.do_quan_trong_id == item.id
                    ? 'color: #408c5b; font-weight: bold'
                    : ''
                "
              >
            ...
            >
              <a
                @click="searchType(item.id, 'phan_cap_id')"
                style="cursor: pointer"
                :style="
      ...
      <div class="right-aside">
        <div class="d-flex">
          <div style="flex: 1; font-size: 18px; font-weight: bold">
            Danh sách vùng đất ngập nước quan trọng
          </div>
          <div class="d-flex">
            <input
              class="form-control"
            ...
            >
              <thead>
                <tr class="footable-header">
                  ...
                  <th style="display: table-cell">Tình trạng</th>
                  <th style="display: table-cell; min-width: 70px">
                    Khu bảo tồn
                  </th>
                  <th style="display: table-cell; min-width: 70px">
                  ...
                  <td>
                    <div style="white-space: initial">
                      <div>
                        ...
                        <div>Diện tích: {{ item.dien_tich }} (m2)</div>
                      </div>
                    </div>
                  </td>
                  <td>{{ item.tinh_trang ? item.tinh_trang.name : "" }}</td>
                  <td>
                    <div>
                      {{
                        item.khu_bao_ton
                          ? item.khu_bao_ton.desig +
                            " " +
                            item.khu_bao_ton.orig_name
                          : ""
                      }}
                    </div>
                  </td>
...
export default {
  ...
  methods: {
    showXoa(data) {
      this.contentDelete =
        "Bạn có chắc chắn muốn xóa khu vực đa dạng sinh học cao " +
        data.ten +
        " không?";
      this.idDelete = data.id;
      this.confirmDelete = true;
      this.delLink = "admin/datngapnuocquantrong/delete/";
    },
    xoaDuLieu() {
      axios
    ...
    showFormAdd() {
      window.location.href = "/admin/datngapnuocquantrong/add";
    },
    showFormEdit(data) {
      window.location.href = "/admin/datngapnuocquantrong/edit/" + data.id;
    },
    getData() {
      axios
        .get(env.endpointhttp + "admin/datngapnuocquantrong/get", {
          params: {
            page: this.page,
            search: this.search,
            tinh_trang_id: this.filter.tinh_trang_id,
            do_quan_trong_id: this.filter.do_quan_trong_id,
    ...
    showXoaBoLoc() {
      if (this.filter.tinh_trang_id == null && this.filter.do_quan_trong_id == null && this.filter.phan_cap_id == null) {
        return false;
      } else return true;
    },
    deleteSearchFilter() {
    ...
    getChiSo() {
      axios
        .get(env.endpointhttp + "admin/datngapnuocquantrong/chiso")
        .then((res) => {
