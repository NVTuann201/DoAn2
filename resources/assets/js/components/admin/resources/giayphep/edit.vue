...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.loaiGiaySelect = Object.assign({}, this.data.loai_giay_phep);
      this.loaiCapSelect = Object.assign({}, this.data.loai_cap);
      this.doiTuongSelect = Object.assign({}, this.data.doi_tuong);
      this.mucDichSelect = Object.assign({}, this.data.muc_dich);
      if (this.data.files) {
        this.form.files = [...JSON.parse(this.data.files)];
      }
      this.fileList = [...this.data.fileList];
      this.changeLoaiGiay(false);
    },
    resetForm() {
    ...
    add() {
      this.$validator.validateAll().then((validate) => {
        this.form.giay_phep_id = this.loaiGiaySelect
          ? this.loaiGiaySelect.id
          : null;
        this.form.loai_cap_phep_id = this.loaiCapSelect
          ? this.loaiCapSelect.id
          : null;
        this.form.doi_tuong_id = this.doiTuongSelect
          ? this.doiTuongSelect.id
          : null;
        this.form.muc_dich_id = this.mucDichSelect
          ? this.mucDichSelect.id
          : null;
        this.disableButton = true;
        if (validate) {
          axios
            .put(env.endpointhttp + "admin/giayphep/editv2", this.form)
            .then(() => {
